...
wss.on('connection', (ws, req) => {
  ...
  ws.roomid = roomid;
  if (rooms[roomid]) {
    ws.send(JSON.stringify({ type: 'info', body: { message: '已在其他地方登录！' } }));
...
function signin(obj, ws) {
  ...
  Signin.create({ room: ws.roomid }, (err, signin) => {
    ...
    ws.signin_timer = setInterval(signin_helper, 50000, signin, ws);
  });
}
...
function signin_helper(signin, ws) {
  ...
  signin.save((err, signin) => {
    if (err) return console.error(err);
    ws.send(JSON.stringify({ type: 'signin_code', body: { key: signin.key, count: signin.containers.length } }));
  });
}
