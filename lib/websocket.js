...
wss.on('connection', (ws, req) => {
  ...
  ws.roomid = roomid;
  if (rooms[roomid]) {
    ws.send(JSON.stringify({ type: 'info', body: { message: '已在其他地方登录！' } }), ack);
  ...
  ws.send(JSON.stringify({ type: 'info', body: { message: '连接成功！' } }));
...
function signin(obj, ws) {
  ...
  Signin.create({ room: ws.roomid }, (err, signin) => {
    ...
    ws.signin_timer = setInterval(signin_helper, 10000, ws);
  });
}

function signin_helper(ws) {
  const key = crypto.createHash('sha256').update(`${Date.now()}${ws.roomid}zhmoll`).digest('hex');
  redis.setSignin(key, ws.signin.id).then(() => {
    ws.send(JSON.stringify({ type: 'signin_code', body: { key: key, count: 'signin.containers.length' } }), ack);
  });
...
function signin_end(obj, ws) {
  if (ws.signin_timer) {
    ...
    delete ws.signin;
    ws.send(JSON.stringify({ type: 'info', body: { message: '签到结束' } }), ack);
  }
}
