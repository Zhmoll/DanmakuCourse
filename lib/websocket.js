...
function handShake_helper(info, cb) {
  ...
  console.log(teacherid, secret, roomid);
...
function signin(obj, ws) {
  ...
  Signin.create({ room: ws.roomid }, (err, signin) => {
    ...
    setImmediate(signin_helper, signin, ws);
    ws.signin_timer = setInterval(signin_helper, 5000, signin, ws);
  });
}

function signin_helper(signin, ws) {
